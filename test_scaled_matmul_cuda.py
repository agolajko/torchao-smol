...
def mm_float8_emulated_block(x, x_scale, y, y_scale, out_dtype) -> torch.Tensor:
    x = x.unflatten(1, (x_scale.shape[1], -1)
                    ).unflatten(0, (x_scale.shape[0], -1))
    y = y.unflatten(1, (y_scale.shape[1], -1)
                    ).unflatten(0, (y_scale.shape[0], -1))
...
class TestFP8Matmul(TestCase):
    ...
    def _test_tautological_mm(self, device: str = "cuda",
        ...
        out_fp8 = scaled_mm_wrap(
            x_fp8, y_fp8, scale_a, scale_b, out_dtype=out_dtype)
    ...
    def test_float8_basics(self, device) -> None:
        ...
        ctx = self.assertRaises(
            ValueError) if torch.version.hip is None and device != "cpu" else contextlib.nullcontext()
        with ctx:
    ...
    def test_float8_scale(self, device) -> None:
        ...
        self.assertEqual(out_fp8.to(torch.float),
                         torch.full(size, 4., device=device))
    ...
    def test_mxfp8_nvfp4_scaled_grouped_mm_2d_2d(self, G, M, N, K, format):
        ...
        X = torch.randn((M, total_K), dtype=torch.bfloat16,
                        device="cuda") * 0.1
        W = torch.randn((N, total_K), dtype=torch.bfloat16,
                        device="cuda") * 0.01
    ...
    def test_mxfp8_scaled_grouped_mm_2d_3d(self, G, M, N, K, format):
        ...
        X = torch.randn((total_M, K), dtype=torch.bfloat16,
                        device="cuda") * 0.1
        ...
        wh, wq, w_blocked_scales, w_global_scales = _3d_to_blocked_scaled(
            W, G, format)
        ...
        def _2d_to_blocked_scaled(X, K, G, offs, format):
            ...
            for i in range(G):
                ...
                if group_size > 0:
                    ...
                    if format == "mxfp8":
                        xh, xq, x_scale = _convert_to_mxfp8_with_hp_ref(
                            x_slice)
                    elif format == "nvfp4":
                        ...
                        xh, xq, x_scale = _convert_to_mxfp4_with_hp_ref(
                            x_slice)
                    else:
                        raise ValueError(
                            f'format must be mxfp8|nvfp4|mxfp4, got "{format}"')
    ...
    def test_scaled_mm_vs_emulated(self, base_dtype):
        ...
        if output_dtype != base_dtype:
            ...
            out_scaled_mm = out_scaled_mm / \
                tensor_to_scale(out_scaled_mm, input_dtype)

            out_emulated = out_emulated.to(compare_type)
            out_emulated = out_emulated / \
                tensor_to_scale(out_emulated, input_dtype)
    ...
    def test_scaled_mm_change_stride(self, base_dtype):
        ...
        x = torch.empty_strided(
            (16, 16), (16, 1), device="cuda", dtype=base_dtype)
        y = torch.empty_strided(
            (16, 32), (1, 64), device="cuda", dtype=base_dtype)
    ...
    def test_float8_bias(self, device) -> None:
        ...
        outb_fp8 = scaled_mm_wrap(
            x, y, scale_a=scale_a, scale_b=scale_b, bias=bias)
        # this fails on ROCm currently because hipblaslt doesn't have amax op
        out_fp32 = out_fp8.to(torch.float32)
        outb_fp32 = outb_fp8.to(torch.float32)
        difference = torch.abs(out_fp32 - outb_fp32)
        self.assertEqual(difference, torch.tensor(
            4.0, device=device).expand_as(out_fp32))
    ...
    def test_float8_bias_relu_edgecase(self, device) -> None:
        ...
        outb_fp32 = outb_fp8.to(torch.float32)
        self.assertEqual(outb_fp32, torch.tensor(-3.0,
                         device=device).expand_as(outb_fp32))
    ...
    def test_error_message_fp8_pre_sm89(self, device) -> None:
        ...
        self.assertRaisesRegex(
            ...
            lambda: scaled_mm_wrap(
                x, y, scale_a, scale_b, out_dtype=torch.float32),
        )
    ...
    def test_float8_rowwise_scaling_sanity(self, device, use_fast_accum: bool) -> None:
        ...
        y_scales = torch.ones(
            (1, y.shape[0]), device=device, dtype=torch.float32)
        ...
        self.assertEqual(
            out_fp8.to(torch.float32), torch.full(
                (M, N), K * (fill_value**2), device=device)
        )
    ...
    def test_float8_error_messages(self, device) -> None:
        ...
        with self.assertRaisesRegex(
            ValueError, re.escape(
                "expected scale_b.stride(1) to be 1, but got 2"),
        ):
            scaled_mm_wrap(
                x_fp8,
                y_fp8,
    ...
    def test_scaled_mm_vs_emulated_row_wise(self, base_dtype):
        ...
        if base_dtype is torch.float32:
            ...
            if _get_torch_cuda_version() < (12, 9):
                raise unittest.SkipTest(
                    "Need CUDA 12.9+ for row-wise fp8 w/ cuBLAS")
            if torch.cuda.get_device_capability() < (9, 0):
                raise unittest.SkipTest(
                    "Need sm90+ for row-wise fp8 w/ cuBLAS")
    ...
    def test_scaled_mm_block_wise_numerics(self, output_dtype, lhs_block, rhs_block, M, N, K, test_case):
        ...
        def _adjust_lhs_scale(x_fp8, x_scales, lhs_block):
            ...
            if lhs_block == 1:
                ...
                assert (
                    x_scales.shape[0] == M and x_scales.shape[1] == K // 128), f"{x_scales.shape=}"
                assert (x_scales.stride(0) == 1 and x_scales.stride(
                    1) in [1, M]), f"{x_scales.stride=}"
        ...
        def _adjust_rhs_scale(y_fp8, y_scales, rhs_block):
            ...
            if rhs_block == 1:
                ...
                assert (
                    y_scales.shape[0] == N and y_scales.shape[1] == K // 128), f"{y_scales.shape=}"
                assert (y_scales.stride(0) == 1 and y_scales.stride(
                    1) in [1, N]), f"{y_scales.stride=}"
        ...
        def _build_lhs(x, lhs_block):
            ...
            x_fp8, x_scales = tensor_to_scale_block(
                x, e4m3_type, lhs_block, 128)
        ...
        def _build_rhs(y, rhs_block):
            ...
            y_fp8, y_scales = tensor_to_scale_block(
                y, e4m3_type, rhs_block, 128)
        ...
        def _run_test(x_hp, x_recipe, x_fp8, x_scales, x_scales_original,
            ...
            self.assertEqual(out_scaled_mm, out_emulated.to(
                output_dtype), atol=atol, rtol=rtol)
        ...
        if test_case == "x_eye_b_eye":
            ...
            x = torch.randint(0, 255, (M, K), device='cuda',
                              dtype=torch.uint8).to(torch.bfloat16)
            y = torch.randint(0, 255, (N, K), device='cuda',
                              dtype=torch.uint8).to(torch.bfloat16)
    ...
    def test_scaled_mm_vs_emulated_block_wise_verify_small_shapes(
        ...
        if lhs_block == 1:
            x_scales = x_scales.t().contiguous().t()
            lhs_recipe = ScalingType.BlockWise1x128
            assert (x_scales.shape[0] == M and x_scales.shape[1]
                    == K // 128), f"{x_scales.shape=}"
            assert (x_scales.stride(0) == 1 and x_scales.stride(
                1) in [1, M]), f"{x_scales.stride=}"
        else:
            lhs_recipe = ScalingType.BlockWise128x128
        ...
        if rhs_block == 1:
            y_scales = y_scales.t().contiguous().t()
            rhs_recipe = ScalingType.BlockWise1x128
            assert (y_scales.shape[0] == N and y_scales.shape[1]
                    == K // 128), f"{y_scales.shape=}"
            assert (y_scales.stride(0) == 1 and y_scales.stride(
                1) in [1, N]), f"{y_scales.stride=}"
        else:
            rhs_recipe = ScalingType.BlockWise128x128
    ...
    def test_honor_sm_carveout(self) -> None:
        ...
        with tempfile.NamedTemporaryFile() as f:
            with torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CUDA]) as prof:
                ...
                scaled_mm_wrap(x_fp8, y_fp8, scale_a=x_scales,
            ...
            if torch.version.hip:
                events = [evt for evt in json.load(
                    open(f.name))["traceEvents"] if evt.get("cat", "") == "kernel"]
    ...
    def test_pack_uint4(self):
        ...
        torch.testing.assert_close(
            lp_data_actual, lp_data_expected, atol=0, rtol=0)
    ...
    def test_blockwise_nvfp4_with_global_scale(self, mkn) -> None:
        ...
        A, A_scale, A_global_scale = data_to_nvfp4_with_global_scale(
            A_ref, BLOCK_SIZE)
        B, B_scale, B_global_scale = data_to_nvfp4_with_global_scale(
            B_ref, BLOCK_SIZE)
    ...
    def test_blockwise_mxfp8_nvfp4_mxfp4_numerics(self, test_case_name, fast_accum, mkn, recipe) -> None:
        ...
        if test_case_name == "a_eye_b_eye":
            ...
            if recipe == "mxfp8":
                ...
                A_ref = torch.randint(0, 255, (M, K), device=device, dtype=torch.uint8).view(
                    torch.float8_e4m3fn).to(torch.bfloat16)
                B_ref = torch.randint(0, 255, (N, K), device=device, dtype=torch.uint8).view(
                    torch.float8_e4m3fn).to(torch.bfloat16)
                ...
                A_ref = _floatx_unpacked_to_f32(
                    torch.randint(0, 16, (M, K), device=device,
                                  dtype=torch.uint8),
                    FP4_EBITS,
                    FP4_MBITS
                ).bfloat16()
                B_ref = _floatx_unpacked_to_f32(
                    torch.randint(0, 16, (N, K), device=device,
                                  dtype=torch.uint8),
                    FP4_EBITS,
                    FP4_MBITS
                ).bfloat16()
            ...
            B_ref = torch.randn((N, K), device=device,
                                dtype=torch.bfloat16) * 1000
            ...
            if recipe == "mxfp8":
                ...
                min_val = -1 * max_val
                A = (A_ref.reshape(-1, BLOCK_SIZE) / A_scale.reshape(M *
                     ceil_div(K, BLOCK_SIZE), 1).float()).reshape(M, K)
                A = A.clamp(min=min_val, max=max_val).to(torch.float8_e4m3fn)
                B = (B_ref.reshape(-1, BLOCK_SIZE) / B_scale.reshape(N *
                     ceil_div(K, BLOCK_SIZE), 1).float()).reshape(N, K)
                ...
                A = (A_ref.reshape(-1, BLOCK_SIZE) / A_scale.reshape(M *
                     ceil_div(K, BLOCK_SIZE), 1).bfloat16()).reshape(M, K)
                A = A.clamp(min=min_val, max=max_val)
                A = _bfloat16_to_float4_e2m1fn_x2(A)
                B = (B_ref.reshape(-1, BLOCK_SIZE) / B_scale.reshape(N *
                     ceil_div(K, BLOCK_SIZE), 1).bfloat16()).reshape(N, K)
    ...
    def test_blockwise_mxfp8_nvfp4_error_messages(self, device, recipe) -> None:
        ...
        expected_a_size = BLOCK_SIZE_MN * \
            ceil_div(M, BLOCK_SIZE_MN) * padded_num_k_blocks
        expected_b_size = BLOCK_SIZE_MN * \
            ceil_div(N, BLOCK_SIZE_MN) * padded_num_k_blocks
        ...
        with self.assertRaisesRegex(
            ...
            non_contiguous_a = torch.ones(
                expected_a_size * 2, device=device, dtype=scale_dtype)[::2]
    ...
    def test_scaled_grouped_gemm_2d_2d(self, fast_accum, strided, wrap_v2):
        ...
        a = torch.randn(m, k * n_groups + k * int(strided),
                        device=device).to(fp8_dtype)[:, :k * n_groups]
        b = torch.randn(n, k * n_groups + k * int(strided),
                        device=device).to(fp8_dtype)[:, :k * n_groups]
        ...
        offs = torch.arange(k, n_groups * k + 1, k,
    ...
    def test_scaled_grouped_gemm_2d_3d(self, fast_accum, strided, wrap_v2):
        ...
        m, n, k, n_groups = 16, 32, 64, 4
        s_int = int(strided)
        a = torch.randn(m * n_groups, k * (1 + s_int),
                        device=device).to(fp8_dtype)[:, :k]
        b = torch.randn(n_groups * (1 + s_int), n, k * (1 + s_int),
                        device=device).to(fp8_dtype)[::(1 + s_int), :, :k]
        ...
        for check_zero_size in (True, False):
            ...
            offs = torch.arange(m, n_groups * m + 1, m,
                                device="cuda", dtype=torch.int32)
            if check_zero_size:
                offs[0] = offs[1]
            scale_a = torch.rand(
                n_groups * m, device="cuda", dtype=torch.float32)
            scale_b = torch.rand(n_groups * n, device="cuda",
                                 dtype=torch.float32).view(n_groups, n)
            f = scaled_grouped_mm_wrap
    ...
    def test_scaled_grouped_gemm_3d_3d(self, fast_accum, strided):
        ...
        m, n, k, n_groups = 16, 32, 64, 4
        s_int = int(strided)
        a = torch.randn(n_groups * (1 + s_int), m, k * (1 + s_int),
                        device=device).to(fp8_dtype)[::(1 + s_int), :, :k]
        b = torch.randn(n_groups * (1 + s_int), n, k * (1 + s_int),
                        device=device).to(fp8_dtype)[::(1 + s_int), :, :k]
        self.assertTrue(a.is_contiguous() is not strided)
        self.assertTrue(b.is_contiguous() is not strided)
        scale_a = torch.rand(n_groups * m, device="cuda",
                             dtype=torch.float32).view(n_groups, m)
        scale_b = torch.rand(n_groups * n, device="cuda",
                             dtype=torch.float32).view(n_groups, n)
    ...
    def test_scaled_grouped_gemm_3d_2d(self, fast_accum, strided):
        ...
        a = torch.randn(n_groups * (1 + s_int), m, k * (1 + s_int),
                        device=device).to(fp8_dtype)[::(1 + s_int), :, :k]
        b = torch.randn(n * n_groups, k * (1 + s_int),
                        device=device).to(fp8_dtype)[:, :k]
        self.assertTrue(a.is_contiguous() is not strided)
        self.assertTrue(b.is_contiguous() is not strided)
        scale_a = torch.rand(n_groups * m, device="cuda",
        ...
        for check_zero_size in (True, False):
            ...
            offs = torch.arange(n, n_groups * n + 1, n,
                                device="cuda", dtype=torch.int32)
            if check_zero_size:
                offs[0] = offs[1]
    ...
    def test_blockwise_mxfp8_compile(self) -> None:
        ...
        A_scale = torch.full((M, ceil_div(K, BLOCK_SIZE)),
                             1.0, device=device, dtype=torch.float8_e8m0fnu)
        B_scale = torch.full((N, ceil_div(K, BLOCK_SIZE)),
                             1.0, device=device, dtype=torch.float8_e8m0fnu)
        C_ref = A_ref @ B_ref.t()
    ...
    def test_blockwise_nvfp4_compile(self) -> None:
        ...
        A_scale = torch.full((M, ceil_div(K, BLOCK_SIZE)),
                             1.0, device=device, dtype=fp4_scaling_dtype)
        B_scale = torch.full((N, ceil_div(K, BLOCK_SIZE)),
                             1.0, device=device, dtype=fp4_scaling_dtype)
        C_ref = A_ref @ B_ref.t()
